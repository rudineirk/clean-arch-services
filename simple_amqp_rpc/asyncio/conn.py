...
class AsyncioAmqpRpcConn:
    def __init__(
        ...
        self.conn = AsyncioAmqpConnection(params)
    ...
    async def send_rpc_call(self, call: RpcCall, timeout=-1) -> RpcResp:
        if timeout == -1:
    ...
    async def on_listen_message(self, msg: AmqpMsg):
        call = decode_rpc_call(msg, self.listen_route)
        resp = await self.rpc_callback(call)
    ...
    async def on_resp_message(self, msg: AmqpMsg):
        try:
