...
class AsyncioAmqpRpc(AsyncioAmqpRpcConn):
    def __init__(
            self,
    ...
    def client(self, service: str, route: str) -> AsyncioRpcClient:
        self.add_publish_route(route)
        return AsyncioRpcClient(self, service, route)

    async def call(self, call: RpcCall) -> RpcResp:
        return await self.send_rpc_call(call)

    async def rpc_callback(self, call: RpcCall) -> RpcResp:
